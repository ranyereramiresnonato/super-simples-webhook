...
{
    /// <summary>
    /// Controller responsável por receber mensagens via webhook do banco NycBank e despachá-las para a fila de processamento.
    /// </summary>
    [Route("api/[controller]")]
    [ApiController]
    public class NycBankController : ControllerBase
    {
        private readonly IGenericDispatcherService _genericDispatcherService;
        public NycBankController(IGenericDispatcherService IGenericDispatcherService)
        {
        ...
        public async Task<IActionResult> Webhook([FromBody] object message)
        {
            ...
            await _genericDispatcherService.DispatchAsync(ProviderIdentifiers.NycBank, message);
