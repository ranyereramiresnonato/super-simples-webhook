...
{
    /// <summary>
    /// Controller responsável por receber mensagens via webhook da Brasil Indoc e despachá-las para a fila de processamento.
    /// </summary>
    [Route("api/[controller]")]
    [ApiController]
    public class BrasilIndocController : ControllerBase
    {
        private readonly IGenericDispatcherService _genericDispatcherService;
        public BrasilIndocController(IGenericDispatcherService IGenericDispatcherService)
        {
        ...
        public async Task<IActionResult> Webhook([FromBody] object message)
        {
            ...
            await _genericDispatcherService.DispatchAsync(ProviderIdentifiers.BrasilIndoc, message);
